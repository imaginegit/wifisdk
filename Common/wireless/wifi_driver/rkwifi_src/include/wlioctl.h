...
typedef struct pm_wake_packet {
	...
	uint8	packet[MAX_WAKE_PACKET_BYTES];
} pm_wake_packet_t;
...
typedef __packed struct sta_prbreq_wps_ie_hdr {
	struct ether_addr staAddr;
	uint16 ieLen;
}  sta_prbreq_wps_ie_hdr_t;

typedef __packed struct sta_prbreq_wps_ie_data {
	sta_prbreq_wps_ie_hdr_t hdr;
	uint8 ieData[1];
}  sta_prbreq_wps_ie_data_t;

typedef __packed struct sta_prbreq_wps_ie_list {
	uint32 totLen;
	uint8 ieDataList[1];
