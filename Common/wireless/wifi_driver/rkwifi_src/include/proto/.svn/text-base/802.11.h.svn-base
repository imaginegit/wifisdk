...
__packed struct dot11_llc_snap_header {
	uint8   dsap;               
	uint8   ssap;               
	uint8   ctl;                
...
__packed struct dot11_rts_frame {
	uint16          fc;     
	uint16          durid;      
...
__packed struct dot11_cts_frame {
	uint16          fc;     
	uint16          durid;      
	struct ether_addr   ra;     
};
...
__packed struct dot11_ack_frame {
	uint16          fc;     
	uint16          durid;      
	struct ether_addr   ra;     
};
...
__packed struct dot11_ps_poll_frame {
	uint16          fc;     
	uint16          durid;      
...
__packed struct dot11_cf_end_frame {
	uint16          fc;     
	uint16          durid;      
	struct ether_addr   ra;     
	struct ether_addr   bssid;      
...
__packed struct dot11_action_wifi_vendor_specific {
	uint8   category;
	uint8   OUI[3];
	uint8   type;
	uint8   subtype;
...
__packed struct dot11_action_vs_frmhdr {
	uint8   category;
	uint8   OUI[3];
	uint8   type;
	uint8   subtype;
...
__packed struct dot11_ctl_header {
	uint16          fc;     
	uint16          durid;      
	struct ether_addr   ra;     
	struct ether_addr   ta;     
...
__packed struct dot11_management_header {
	uint16          fc;     
	uint16          durid;      
...
__packed struct dot11_assoc_req {
	uint16          capability; 
	uint16          listen;     
};
#define DOT11_ASSOC_REQ_FIXED_LEN   4   

__packed struct dot11_reassoc_req {
	uint16          capability; 
	uint16          listen;     
...
__packed struct dot11_assoc_resp {
	uint16          capability; 
	uint16          status;     
	uint16          aid;        
};
...
__packed struct dot11_action_measure {
	uint8   category;
	uint8   action;
	uint8   token;
	uint8   data[1];
...
__packed struct dot11_action_ht_ch_width {
	uint8   category;
	uint8   action;
	uint8   ch_width;
};

__packed struct dot11_action_ht_mimops {
	uint8   category;
	uint8   action;
	uint8   control;
};
...
__packed struct dot11_power_cnst {
	uint8 id;
	uint8 len;
	uint8 power;
};
...
__packed struct dot11_power_cap {
	uint8 min;
	uint8 max;
};
typedef struct dot11_power_cap dot11_power_cap_t;

__packed struct dot11_tpc_rep {
	uint8 id;
	uint8 len;
	uint8 tx_pwr;
	uint8 margin;
...
__packed struct dot11_supp_channels {
	uint8 id;
	uint8 len;
	uint8 first_channel;
	uint8 num_channels;
...
__packed struct dot11_brcm_extch {
	uint8   id;     
	uint8   len;        
	uint8   oui[3];     
	uint8   type;           
...
__packed struct dot11_action_frmhdr {
	uint8   category;
	uint8   action;
	uint8   data[1];
};
...
__packed struct dot11_channel_switch {
	uint8 id;   
	uint8 len;  
	uint8 mode; 
	uint8 channel;  
...
__packed struct dot11_action_switch_channel {
	uint8   category;
	uint8   action;
...
__packed struct dot11_csa_body {
	uint8 mode; 
	uint8 reg;  
	uint8 channel;  
	uint8 count;    
...
__packed struct dot11_action_ext_csa {
	uint8   category;
	uint8   action;
	dot11_ext_csa_ie_t chan_switch_ie;  
};

__packed struct dot11y_action_ext_csa {
	uint8   category;
	uint8   action;
	struct dot11_csa_body b;    
};

__packed struct dot11_obss_coex {
	uint8   id;
	uint8   len;
	uint8   info;
};
...
__packed struct dot11_obss_chanlist {
	uint8   id;
	uint8   len;
	uint8   regclass;
	uint8   chanlist[1];
...
__packed struct dot11_extcap_ie {
	uint8 id;
	uint8 len;
	uint8 cap[1];
};
...
__packed struct dot11_meas_req {
	uint8 id;
	uint8 len;
	uint8 token;
	uint8 mode;
...
__packed struct dot11_meas_rep {
	uint8 id;
	uint8 len;
	uint8 token;
	uint8 mode;
...
__packed struct dot11_meas_rep_basic {
	uint8 channel;
	uint8 start_time[8];
	uint16 duration;
	uint8 map;
...
__packed struct chan_map_tuple {
	uint8 channel;
	uint8 map;
};
typedef struct chan_map_tuple chan_map_tuple_t;

__packed struct dot11_ibss_dfs {
	uint8 id;
	uint8 len;
...
__packed struct qos_cap_ie {
	uint8 qosinfo;
};
typedef struct qos_cap_ie qos_cap_ie_t;

__packed struct dot11_qbss_load_ie {
	uint8 id;           
	uint8 length;
...
__packed struct dot11_management_notification {
	uint8 category;         
	uint8 action;
	uint8 token;
	uint8 status;
...
__packed struct dot11_bss_trans_query {
	uint8 category;				
	uint8 action;				
	uint8 token;				
	uint8 reason;				
...
__packed struct dot11_bss_trans_req {
	uint8 category;				
	uint8 action;				
	uint8 token;				
	uint8 reqmode;				
...
__packed struct dot11_bss_trans_res {
	uint8 category;				
	uint8 action;				
	uint8 token;				
	uint8 status;				
...
__packed struct dot11_addba_req {
	uint8 category;             
	uint8 action;               
	uint8 token;                
...
__packed struct dot11_addba_resp {
	uint8 category;             
	uint8 action;               
	uint8 token;                
	uint16 status;              
...
__packed struct dot11_rrm_cap_ie {
...
__packed struct dot11_ext_cap_ie {
...
__packed struct dot11_rm_action {
	uint8 category;             
	uint8 action;               
	uint8 token;                
	uint8 data[1];
...
__packed struct dot11_rmreq_bcn {
	uint8 id;
	uint8 len;
	uint8 token;
	uint8 mode;
...
__packed struct dot11_rmrep_bcn {
	uint8 reg;
	uint8 channel;
	uint32 starttime[2];
	uint16 duration;
...
__packed struct dot11_rmrep_nbr {
	struct ether_addr   bssid;
	uint32  bssid_info;
	uint8 reg;
	uint8 channel;
	uint8 phytype;
	uint8 sub_elements[1]; 	
};
...
__packed struct ht_prop_cap_ie {
	uint8   id;     
	uint8   len;        
	uint8   oui[3];     
	uint8   type;           
...
__packed struct ht_prop_add_ie {
	uint8   id;     
	uint8   len;        
	uint8   oui[3];     
	uint8   type;       
...
__packed struct channel_switch_timing_ie {
	uint8 id;
	uint8 len;
	uint16 switch_time;		
...
__packed struct pti_control_ie {
	uint8 id;
	uint8 len;
	uint8 tid;
	uint16 seq_control;
...
__packed struct pu_buffer_status_ie {
	uint8 id;
	uint8 len;
	uint8 status;
};
