...
#define IW_ESSID_MAX_SIZE	ETHNAMELEN
...
struct	iw_scan_req
{
	unsigned char		scan_type; /* IW_SCAN_TYPE_{ACTIVE,PASSIVE} */
	unsigned char		essid_len;
	unsigned char		num_channels; /* num entries in channel_list;
				       * 0 = scan all allowed channels */
	unsigned char		flags; /* reserved as padding; use zero, this may
	 ...
	 * max_channel_time.
	 */
	unsigned int		min_channel_time; /* in TU */
	unsigned int		max_channel_time; /* in TU */
...
struct	iw_michaelmicfailure
{
	...
	unsigned char		tsc[IW_ENCODE_SEQ_MAX_SIZE]; /* LSB first */
};
...
struct	iw_pmkid_cand
{
	unsigned int		flags; /* IW_PMKID_CAND_* */
	unsigned int		index; /* the smaller the index, the higher the
				* priority */
	struct sockaddr	bssid;
};
...
struct	iw_range
{
	...
	signed int		bitrate[IW_MAX_BITRATES];	/* list, in bps */

	/* RTS threshold */
	signed int		min_rts;	/* Minimal RTS threshold */
	signed int		max_rts;	/* Maximal RTS threshold */

	/* Frag threshold */
	signed int		min_frag;	/* Minimal frag threshold */
	signed int		max_frag;	/* Maximal frag threshold */

	/* Power Management duration & timeout */
	signed int		min_pmp;	/* Minimal PM period */
	signed int		max_pmp;	/* Maximal PM period */
	signed int		min_pmt;	/* Minimal PM timeout */
	signed int		max_pmt;	/* Maximal PM timeout */
	...
	signed int		txpower[IW_MAX_TXPOWER];	/* list, in bps */
	...
	signed int		min_retry;	/* Minimal number of retries */
	signed int		max_retry;	/* Maximal number of retries */
	signed int		min_r_time;	/* Minimal retry lifetime */
	signed int		max_r_time;	/* Maximal retry lifetime */
...
#define IW_EV_COMPAT_UINT_LEN	(IW_EV_COMPAT_LCP_LEN + sizeof(unsigned int))
...
#define IW_EV_UINT_PK_LEN	(IW_EV_LCP_PK_LEN + sizeof(unsigned int))
